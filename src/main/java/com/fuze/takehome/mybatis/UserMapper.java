...
public interface UserMapper {
	...
	@Select("SELECT "
			+ "id AS userId, customer_id, username, first_name, last_name, email, telephone_number, mobile_number, fax_number, active "
			+ "FROM takeHome.users "
			+ "WHERE id = #{id}")
	@Results(value = {
			...
			@Result(property = "departmentIds", column = "userId", javaType = Collection.class, many = @Many(select = "selectUserDepartments")),
			@Result(property = "active", column = "active")
	})
	public User read(Long id);

	@Select("SELECT department_id FROM takeHome.user_department WHERE user_id = #{userId}")
