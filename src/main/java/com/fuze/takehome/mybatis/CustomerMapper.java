...
public interface CustomerMapper {
	...
	@Select("SELECT c.id,c.name,c.active,c.contact_email,c.contact_firstname,c.contact_lastname "
			+ "FROM takeHome.customers c " + "WHERE c.id = #{id}")
	...
	@Insert("UPDATE takeHome.customers SET name = #{in.name}, active = #{in.active}, contact_email = #{in.contact.email},"
			+ " contact_first_name = #{in.contact.firstName}, contact_last_name = #{in.contact.lastName} WHERE id = #{id}")
